...
/**
 * 尊重原创感谢开源：本控件是由Wharton的StickyHeadListView(https://github.com/emilsjolander/
 * StickyListHeaders)项目修改而来，主要是根据项目需求做了一些配置与完善滚动时的效率问题。<br>
 * 
 * 一个伪造的ListView，实际是一个FrameLayout包装了staickyHead和ListView
 * 
 * @author Jake Wharton (jakewharton@gmail.com)
 * @author kymjs (https://github.com/kymjs)
 * @since 2015-3
 */
public class StickyHeadListView extends FrameLayout {

    public interface OnHeaderClickListener {
        void onHeaderClick(StickyHeadListView l, View header, int itemPosition,
                String headerId, boolean currentlySticky);
    }
    ...
    public interface OnHeadChangeListener {
         ...
         *            however guaranteed that this view has been measured, there
         *            for you should user getMeasured* methods instead of get*
         *            methods for determining the view's size.
         * @param offset
         *            The amount the sticky header is offset by towards to top
         *            of the screen.
         */
        void onStickyHeaderOffsetChanged(StickyHeadListView l, View header,
                int offset);

        /**
         * stickyHead改变完成之后被触发的监听器
         * 
         * @param l
         *            The view parent
         * @param header
         ...
         */
        void onStickyHeaderChanged(StickyHeadListView l, View header,
                int itemPosition, String headerId);

    }

    /* 这个容器包含了一个ListView和一个stickyHead */
    ...
    private OnHeadChangeListener mOnStickyHeaderChangedListener;
    private AdapterWrapperDataSetObserver mDataSetObserver;
    private Drawable mDivider;
    private int mDividerHeight;
    private boolean openStickyHead = true;

    public StickyHeadListView(Context context) {
        this(context, null);
    }

    public StickyHeadListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public StickyHeadListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                    R.styleable.StickyHeadListView, 0, 0);

            try {
                // -- View attributes --
                int padding = a.getDimensionPixelSize(
                        R.styleable.StickyHeadListView_android_padding, 0);
                mPaddingLeft = a.getDimensionPixelSize(
                        R.styleable.StickyHeadListView_android_paddingLeft,
                        padding);
                mPaddingTop = a.getDimensionPixelSize(
                        R.styleable.StickyHeadListView_android_paddingTop,
                        padding);
                mPaddingRight = a.getDimensionPixelSize(
                        R.styleable.StickyHeadListView_android_paddingRight,
                        padding);
                mPaddingBottom = a.getDimensionPixelSize(
                        R.styleable.StickyHeadListView_android_paddingBottom,
                        padding);
                ...
                mClippingToPadding = a.getBoolean(
                        R.styleable.StickyHeadListView_android_clipToPadding,
                        true);
                ...
                final int scrollBars = a.getInt(
                        R.styleable.StickyHeadListView_android_scrollbars,
                        0x00000200);
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    mList.setOverScrollMode(a
                            .getInt(R.styleable.StickyHeadListView_android_overScrollMode,
                                    0));
                }
                ...
                mList.setFadingEdgeLength(a
                        .getDimensionPixelSize(
                                R.styleable.StickyHeadListView_android_fadingEdgeLength,
                                mList.getVerticalFadingEdgeLength()));
                final int fadingEdge = a
                        .getInt(R.styleable.StickyHeadListView_android_requiresFadingEdge,
                                0);
                ...
                mList.setCacheColorHint(a.getColor(
                        R.styleable.StickyHeadListView_android_cacheColorHint,
                        mList.getCacheColorHint()));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    mList.setChoiceMode(a.getInt(
                            R.styleable.StickyHeadListView_android_choiceMode,
                            mList.getChoiceMode()));
                }
                mList.setDrawSelectorOnTop(a
                        .getBoolean(
                                R.styleable.StickyHeadListView_android_drawSelectorOnTop,
                                false));
                mList.setFastScrollEnabled(a
                        .getBoolean(
                                R.styleable.StickyHeadListView_android_fastScrollEnabled,
                                mList.isFastScrollEnabled()));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    mList.setFastScrollAlwaysVisible(a
                            .getBoolean(
                                    R.styleable.StickyHeadListView_android_fastScrollAlwaysVisible,
                                    mList.isFastScrollAlwaysVisible()));
                }

                mList.setScrollBarStyle(a.getInt(
                        R.styleable.StickyHeadListView_android_scrollbarStyle,
                        0));

                if (a.hasValue(R.styleable.StickyHeadListView_android_listSelector)) {
                    mList.setSelector(a
                            .getDrawable(R.styleable.StickyHeadListView_android_listSelector));
                }

                mList.setScrollingCacheEnabled(a.getBoolean(
                        R.styleable.StickyHeadListView_android_scrollingCache,
                        mList.isScrollingCacheEnabled()));

                if (a.hasValue(R.styleable.StickyHeadListView_android_divider)) {
                    mDivider = a
                            .getDrawable(R.styleable.StickyHeadListView_android_divider);
                }

                mDividerHeight = a.getDimensionPixelSize(
                        R.styleable.StickyHeadListView_android_dividerHeight,
                        mDividerHeight);

                mList.setTranscriptMode(a.getInt(
                        R.styleable.StickyHeadListView_android_transcriptMode,
                        ListView.TRANSCRIPT_MODE_DISABLED));

                // -- StickyListHeaders attributes --
                mAreHeadersSticky = a.getBoolean(
                        R.styleable.StickyHeadListView_hasStickyHeaders, true);
                mIsDrawingListUnderStickyHeader = a
                        .getBoolean(
                                R.styleable.StickyHeadListView_isDrawingListUnderStickyHeader,
                                true);
            } finally {
                a.recycle();
            }
            ...
            mHeader.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    mOnHeaderClickListener.onHeaderClick(
                            StickyHeadListView.this, mHeader, mHeaderPosition,
                            mHeaderId, true);
                }
            });
        }
    ...
    private void setHeaderOffet(int offset) {
        if (mHeaderOffset == null || mHeaderOffset != offset) {
            ...
            if (mOnStickyHeaderChangedListener != null) {
                mOnStickyHeaderChangedListener.onStickyHeaderOffsetChanged(
                        this, mHeader, -mHeaderOffset);
            }
        }
    }
    ...
    private class AdapterWrapperHeaderClickHandler implements
        ...
        public void onHeaderClick(View header, int itemPosition, String headerId) {
            mOnHeaderClickListener.onHeaderClick(StickyHeadListView.this,
    ...
    private boolean isStartOfSection(int position) {
        return position == 0
                || !mAdapter.getHeaderId(position).equals(
                        mAdapter.getHeaderId(position - 1));
    }
            ...
            if (mOnHeaderClickListener != null) {
                ...
                if (mHeader != null) {
                    mHeader.setOnClickListener(new OnClickListener() {
                        ...
                        public void onClick(View v) {
                            mOnHeaderClickListener.onHeaderClick(
                                    StickyHeadListView.this, mHeader,
    ...
    public void setAdapter(StickyHeadAdapter adapter) {
        if (adapter == null) {
    ...
    public StickyHeadAdapter getAdapter() {
        ...
        if (l != null) {
            mList.setOnTouchListener(new OnTouchListener() {
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    return l.onTouch(StickyHeadListView.this, event);
                }
            });
        } else {
    ...
    public boolean isOpenStickyHead() {
        return openStickyHead;
    }

    public void setOpenStickyHead(boolean openStickyHead) {
        this.openStickyHead = openStickyHead;
    }
}
